...
20. **`test_large_image_resize_preserves_aspect_ratio`**
    - 기존 테스트보다 큰 1280x720 입력도 최대 가로 크기 기준으로 320x180까지 비율 유지 축소되는지 확인
    - `ConvertStats` 의 원본/출력 크기와 실제 출력 이미지 크기를 함께 검증

### 🎨 JPEG 배경색 테스트

21. **`test_jpeg_background_flattens_transparency`**
    - 투명 PNG 를 JPEG 로 변환할 때 지정한 배경색이 투명 영역에 적용되는지 확인

22. **`flatten_for_jpeg_*`** (2개)
    - 완전 투명 픽셀은 배경색으로 합성되고, 완전 불투명 픽셀은 원본 색을 유지하는지 확인
    - 부분 투명 픽셀이 알파 값에 맞춰 배경색과 섞이는지 확인

### 🧯 안정성 / 실패 격리 테스트

23. **`test_single_conversion_rejects_corrupt_image`**
    - 확장자는 이미지지만 내용이 손상된 파일을 단일 변환할 때 `ConverterError::Image` 로 실패하는지 확인
    - 디코딩 실패 시 출력 파일을 만들지 않는지 검증

24. **`test_batch_continues_when_image_file_is_corrupt`**
    - 정상 이미지와 손상 이미지가 같은 입력 디렉토리에 있을 때 정상 파일은 변환하고 손상 파일만 실패 집계하는지 확인
    - 손상 파일에 대응하는 출력 파일이 생성되지 않는지 검증
...
- ✅ 비이미지 파일 자동 스킵
- ✅ 손상 이미지 입력 실패 처리 (단일 변환 출력 미생성 / 일괄 변환 실패 격리)
- ✅ 빈 디렉토리 처리
