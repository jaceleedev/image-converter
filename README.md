...
1. 변환 대상 선택 (이미지 1개 / 폴더 전체)
2. 입력 경로 입력 (모드에 따라 파일 또는 폴더)
3. (폴더 모드만) 하위 폴더 포함 여부
4. 출력 형식 선택 (WebP 웹 권장 / AVIF 작은 용량 / PNG 무손실 / JPEG 사진 호환성)
5. 품질 선택 (웹 권장/균형/작게 저장/원본에 가깝게/직접 입력 — PNG 출력은 무손실이라 자동 스킵)
6. (JPEG 출력만) 투명 영역 배경색 선택 (흰색 기본 / 검정 / 직접 입력 #RRGGBB)
7. 크기 줄이기 선택 (최대 가로 크기 px, 원본보다 작을 때만 적용)
8. (폴더 모드만) 동시 변환 스레드 수 (비워두면 자동)
9. 저장 경로 확인 (단일 파일은 선택한 형식과 확장자 일치 여부 검증)

단일 파일 변환의 기본 출력 경로는 입력 파일과 같은 폴더에서 확장자만 바꾼 값을 먼저 제안합니다. 예를 들어 `cat.png` 를 WebP 로 바꾸면 `cat.webp` 가 기본값이고, 이미 같은 이름의 파일이 있으면 `cat_converted.webp`, `cat_converted_2.webp` 순서로 충돌을 피합니다.

대화형 모드에서는 이미지 1개와 폴더 전체 변환 모두에서 최대 가로 크기를 지정해 리사이즈할 수 있습니다. 원본보다 작은 값일 때만 축소하고, 작은 이미지는 확대하지 않습니다.

JPEG 출력에서는 투명 PNG/WebP 의 투명 영역을 배경색 위에 합성합니다. 기본값은 웹 페이지에서 가장 무난한 흰색이며, 필요하면 검정이나 `#RRGGBB` 형식의 직접 입력 색상을 선택할 수 있습니다.
...
단일 파일 변환에서는 `-f webp -o photo.jpg` 처럼 선택한 출력 형식과 출력 파일 확장자가 다르면 변환을 시작하지 않습니다. JPEG 는 `.jpg` 와 `.jpeg` 를 모두 허용합니다.
